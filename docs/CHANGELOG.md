# Changelog — Megawide WMS (PRC-WH APP)

Session-by-session record of what changed and why. Split out of `CLAUDE.md` on
2026-09-10: that file is re-read in full at the start of every turn, and at 3,001 lines
the changelog alone was about 51,000 tokens of it — roughly a third of the session's
whole cost, compounding with every entry. `CLAUDE.md` now keeps only what is needed
every turn (workflow, architecture, security rules, current state) and this file holds
the history, read on demand.

**Standing workflow is unchanged:** every prompt that changes anything appends an entry
HERE, then `git add -A && git commit` and push.
...
### 2026-09-10 — Session: `CLAUDE.md` split — the changelog moves here

No app code changed. This is a token-cost fix, and it was measured before it was made.

**The measurement.** The session transcript reports 613 assistant turns across three
prompts, with an effective weighted total of about 49.5M tokens (cache reads at 0.1x,
cache writes at 2x, output at 5x). `CLAUDE.md` stood at 3,001 lines / 204,021 chars /
~51,005 tokens, and a project instructions file is re-read in full at the START OF EVERY
TURN — so that one file was **34% of the entire session's cost**, and it grew with every
prompt, because the standing workflow says to append an entry each time. The cost was
compounding on itself.

**The split.** Lines 1–113 stay in `CLAUDE.md` — project, standing workflow, current
state, data architecture, deployment, known blockers, commands. All 46 changelog entries
moved to `docs/CHANGELOG.md`, byte-for-byte, newest last, under a header explaining why.
`CLAUDE.md` is now 7,774 chars (~1,950 tokens), a **96% cut** in what is paid per turn.

Nothing is lost: the history is still committed, still complete, still in the repo, and
`CLAUDE.md`'s own Changelog section now points at it and says what belongs where — facts
that stay true in `CLAUDE.md`, narrative of what a session did in `docs/CHANGELOG.md`.
The reasoning, the already-fixed bugs and the measurement traps are all still there to be
read or grepped; they just are not re-read when nobody asked for them.

**The other four cost groups measured, recorded so they are not repeated:**
- **File work, 36%** — the largest group, and self-inflicted. Editing source through
  throwaway patch scripts (`b2.cjs`, `b3.cjs`) cost ~56,000 tokens of script text, and
  several failed their own `must()` guards on inexact comment matches, forcing a read, an
  edit, a re-run and finally abandonment for direct edits anyway. A changed file is also
  re-dumped into the conversation whole — 21 times, another ~86,000 tokens. **Use direct
  edits; a patch script is only worth it for a change too repetitive to edit by hand.**
- **Screenshots, 10%** — 15 requested, **9 came back blank**, because the browser pane
  does not composite while hidden. That limitation is recorded in five earlier entries in
  this very file. They still cost image payload. **Do not screenshot unless the pane is
  confirmed visible; DOM measurement was doing the real work regardless.**
- **Browser checks, 10%** — 142 `javascript_tool` calls. The probes returned full detail
  arrays (every bar's geometry) where only a count and the failures were needed. **Return
